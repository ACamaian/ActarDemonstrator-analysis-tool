...
int main (int argc, char ** argv){
  ...
  TString fname="mc_data_sho_pr1.dat";
  ...
  TFile *fout = new TFile (Form("../data/%s.root",fname.Data()),"RECREATE");
  
  
  TTree *tout = new TTree("EventTree_0","Simulated Event Tree with XYZ positions ");
  ...
  while(getline(in,line)){
	...
	trigger=0;
      ...
      continue;
        ...
        if(entry>0){ //Save the event when reading the n.ev line of the following one
	if(trigger>=10){
		event->setIsgood(true);
		event->setIDCode(1);	
		}
